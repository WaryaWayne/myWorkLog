...
const workDoneFile = join(process.cwd(), "results", "daily_journal.md");
...
for (const section of sections) {
  ...
  const { text } = await generateText({
    ...
    system:
      "Summarize and explain the daily progress from the following commits in a well-formatted markdown format suitable for GitHub. Use headings, bullet points, and code blocks where appropriate to make it readable and professional.",
    prompt: commits,
  });
